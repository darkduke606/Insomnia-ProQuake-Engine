...
void VID_RememberWindowPos (void) {
	extern cvar_t vid_window_x, vid_window_y;
	RECT	rect;

	if (GetWindowRect (mainwindow, &rect)) {
		if (rect.left < GetSystemMetrics (SM_CXSCREEN) && rect.top < GetSystemMetrics (SM_CYSCREEN)  &&
			rect.right > 0 &&  rect.bottom > 0) {
			Cvar_SetValueByRef (&vid_window_x, (float)rect.left);
			Cvar_SetValueByRef (&vid_window_y, (float)rect.top);
		}
	}
}*/
...
void VID_CheckWindowXY (void) {
	...
	if (((int)vid_window_x.value > (GetSystemMetrics (SM_CXSCREEN) - 160)) ||
		...
		Cvar_SetValueByRef (&vid_window_x, 0.0);
		Cvar_SetValueByRef (&vid_window_y, 0.0 );
	}
}
#endif
...
int VID_SetMode (int modenum, unsigned char *palette) {
	...
	while ((modenum >= nummodes) || (modenum < 0)) {
		if (vid_modenum == NO_MODE) {
			...
			Cvar_SetValueByRef (&vid_mode, (float)modenum);
		}
		else {
			Cvar_SetValueByRef (&vid_mode, (float)vid_modenum);
			return 0;
		}
	}
...
/*			if (COM_CheckParm ("-resetwinpos"))
			...
			if ((_vid_default_mode_win.value < 0) || (_vid_default_mode_win.value >= nummodes))
				Cvar_SetValueByRef (&_vid_default_mode_win, windowed_default);

			Cvar_SetValueByRef (&vid_mode, _vid_default_mode_win.value);
		}
	}
...
void VID_MenuKey (int key) {
	...
	case 'd':
		S_LocalSound ("misc/menu1.wav");
		firstupdate = 0;
		Cvar_SetValueByRef (&_vid_default_mode_win, vid_modenum);
		break;
